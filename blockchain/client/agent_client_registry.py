...
class AgentRegistryConfig(BaseModel):
    """Constructed once at service startup from wherever your config
    actually lives (env vars, a secrets manager, a settings object) —
    this class doesn't care which. Compare to the earlier version, which
    read os.environ["REGISTRAR_PRIVATE_KEY"] directly inside __init__,
    making the client impossible to unit test without patching the
    process environment."""
...
class AgentRecord(BaseModel):
    ...
    state: int  # LifecycleState enum index — see AgentRegistry.sol's LifecycleState
...
class AgentRegistryClient:
    def __init__(self, config: AgentRegistryConfig, rpc_provider_manager, **base_client_kwargs):
        self._client = BaseContractClient(
            ...
            **base_client_kwargs,  # lets a caller inject a shared NonceManager/ContractCache/MetricsCollector, see blockchain_client.py
        )
